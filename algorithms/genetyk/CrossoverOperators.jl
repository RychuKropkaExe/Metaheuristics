...
function one_point(a::Array{Int}, b::Array{Int})::Array{Array{Int}}
    path_a::Array{Int} = copy(a)
    path_a.=0
    path_b::Array{Int} = copy(b)
    path_b.=0
...
function op_crossover(parents::Array{Tuple{Chromosome,Chromosome}})::Array{Array{Int}}
    children_paths::Array{Array{Int}} = []
    for pair in parents
        parent_a::Chromosome = pair[1]
        parent_b::Chromosome = pair[2]
        append!(children_paths, one_point(parent_a.path, parent_b.path))
    end
    return children_paths
end